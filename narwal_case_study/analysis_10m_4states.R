...
hist(data$maxdep+10, breaks = 100, prob = TRUE, bor = "white", ylim = c(0, 0.01),
     xlim = c(10, 1000),
     main = "Distribution of maximum depth", xlab = "Max depth", xaxt = "n")
axis(1, at = c(10, 500, 1000), labels = c(10, 500, 1000))
curve(0.6 * dgamma2(x, 20, 30), add = TRUE, col = color[1], lwd = 2)
curve(0.1 * dgamma2(x, 50, 50), add = TRUE, col = color[2], lwd = 2)
curve(0.15 * dgamma2(x, 350, 70), add = TRUE, col = color[3], lwd = 2)
curve(0.1 * dgamma2(x, 500, 70), add = TRUE, col = color[4], lwd = 2)
...
nll = function(par){
  ...
  lallprobs = matrix(0, length(maxdep), N)
  ind = which(!is.na(maxdep)) # NA handling
  for(j in 1:N) {
    lallprobs[ind,j] <- dgamma2(maxdep[ind], mu[j], sigma[j], log = TRUE)
  }
  
  ## forward algorithm
  -forward(delta, Gamma, lallprobs, logspace = TRUE)
}
...
eta_base = rep(-3, 12)
logmu_base = log(c(10, 50, 280, 450))
logsigma_base = log(c(20, 50, 100, 100))
...
#   list(
#     eta = rnorm(12, eta_base, 0.4),
#     logmu = sort(rnorm(4, logmu_base, 0.4)),
#     logsigma = sort(rnorm(4, logsigma_base, 0.3))
#   )
# })
# 
...
# plot(llks)
# # highest log-likelihood is very weird model -> something numerically weird.
# # most agree in optimum llk = -23590.3 -> choose this one by using second best
# 
# # refit best model
# # optpar = init_values[[which.max(llks)]]
# optpar <- init_values[[order(llks, decreasing = TRUE)[2]]]
...
# saveRDS(mod_par, "./narwal_case_study/objects/mod_normal4_10m.rds")
mod_par = readRDS("./narwal_case_study/objects/mod_normal4_10m.rds")
...
logmu_base = log(c(10, 30, 230, 480))
logsigma_base = log(c(10, 30, 100, 200))
...
init_values <- lapply(1:n_init, function(i) {
  list(
    eta = rnorm(12, eta_base, 0.4),
    logmu = sort(rnorm(4, logmu_base, 0.3)),
    logsigma = sort(rnorm(4, logsigma_base, 0.3))
  )
})
...
for(i in 1:n_init){
  ...
  lambdas[[i]] <- runif(4, 5, 25) # initial smoothenss penalty parameters
...
saveRDS(mod_np, "./narwal_case_study/objects/mod_unconstrained_opt4_10m.rds") # saving the model")
mod_np = readRDS("./narwal_case_study/objects/mod_unconstrained_opt4_10m.rds") # reading the model


hist(data$maxdep+10, breaks = 100, prob = TRUE, bor = "white", ylim = c(0, 0.01),
     xlim = c(10, 1000),
     main = "Distribution of maximum depth", xlab = "Max depth", xaxt = "n")
axis(1, at = c(10, 250, 500, 750, 1000), labels = c(10, 250, 500, 750, 1000))
for(i in 1:4) lines(mod_np$xseq+10, mod_np$delta[i] * mod_np$dens[,i], lwd = 2, col = color[i])
...
dat$lambda = mod_np$lambda * 50 # increase it a bit to avoid overfitting
...
# save the model
saveRDS(mod_uni, "./narwal_case_study/objects/mod_constrained_opt4_10m.rds")
mod_uni = readRDS("./narwal_case_study/objects/mod_constrained_opt4_10m.rds") # reading the model
...
# pdf("./narwal_case_study/figures/narwhal_statedep4_10m.pdf", width = 6, height = 5.5)
...
hist(data$maxdep+10, breaks = breaks, prob = TRUE, bor = "white", 
     xlim = c(10, 1000), ylim = c(0, 0.002), las = 1, ylab = "",
     main = "", xaxt = "n")
axis(1, at = c(10, 200, 400, 600, 800, 1000), labels = c(10, 200, 400, 600, 800, 1000))
...
for(i in 1:4) curve(mod_par$delta[i] * dgamma2(x, mod_par$mu[i], mod_par$sigma[i]), 
                    col = color[i], lwd = lwd, lty = ltys[i], add = TRUE, from = 0, to = 1000, n = 500)
...
for(i in 1:4) curve(mod_par$delta[i] * dgamma2(x, mod_par$mu[i], mod_par$sigma[i]), 
                    col = color[i], lwd = lwd, lty = ltys[i], add = TRUE, from = 550, to = 1000)
box()
...
for(i in 1:4) lines(mod_np$xseq[idx]+10, mod_np$delta[i] * mod_np$dens[idx,i], 
...
for(i in 1:4) lines(mod_uni$xseq[idx]+10, mod_uni$delta[i] * mod_uni$dens[idx,i], 
                    lwd = lwd, col = color[c(1,2,4,3)[i]], lty = ltys[c(1,2,4,3)[i]])
rect(580, -0.0001/5, 1020, 0.0001)
...
plot(idx, data$maxdep[idx], type = "h", col = color[mod_par$states[idx]],
